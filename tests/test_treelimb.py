#!/usr/bin/env python3
"""
Simple regression tests for treelimb.
"""
...
from src import treelimb


class TestTreelimb(unittest.TestCase):

    def test_basic_logging(self):
        """Test that basic logging produces expected format."""
        logger = treelimb.get_logger("test", to_stderr=False)
        logger.info("test message")
        
        # Get the log file and read its contents
        log_file = treelimb.get_log_file(logger)
        ...
        self.assertIn("test_treelimb.py:", output)
    ...
    def test_log_stack_trace(self):
        ...
        logger = treelimb.get_logger("test_stack", to_stderr=False)
        treelimb.log_stack_trace(logger, "debug point")
    ...
    def test_die_exits(self):
        ...
        logger = treelimb.get_logger("test_die")
        
        with self.assertRaises(SystemExit) as cm:
            treelimb.die(logger, "fatal error (not really, just testing)")
        
        self.assertEqual(cm.exception.code, 1)

    def test_message_formatting(self):
        """Test that message % args works correctly."""
        logger = treelimb.get_logger("test_format", to_stderr=False)
        treelimb.log_stack_trace(logger, "value is %d", 42)
